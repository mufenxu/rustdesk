...
const val DEFAULT_NOTIFY_TITLE = "万联远程"
...
class MainService : Service() {
    ...
    private val wakeLock: PowerManager.WakeLock by lazy { powerManager.newWakeLock(PowerManager.ACQUIRE_CAUSES_WAKEUP or PowerManager.SCREEN_BRIGHT_WAKE_LOCK, "wanlian:wakelock")}

    // Partial wakelock keeps CPU alive when screen is off, preventing Doze mode
    // from killing the network connection (which causes 30s timeout disconnect)
    private val partialWakeLock: PowerManager.WakeLock by lazy {
        powerManager.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "wanlian:connection")
    }
    // WiFi lock prevents WiFi from going to low-power mode in background
    private val wifiLock: WifiManager.WifiLock by lazy {
        (applicationContext.getSystemService(Context.WIFI_SERVICE) as WifiManager)
            .createWifiLock(WifiManager.WIFI_MODE_FULL_HIGH_PERF, "wanlian:wifi")
    }
    ...
    fun startCapture(): Boolean {
        ...
        // Acquire partial wakelock and wifi lock to keep connection alive in background
        if (!partialWakeLock.isHeld) {
            partialWakeLock.acquire()
            Log.d(logTag, "Partial wakelock acquired for background connection")
        }
        if (!wifiLock.isHeld) {
            wifiLock.acquire()
            Log.d(logTag, "WiFi lock acquired for background connection")
        }
        return true
    }
    ...
    fun stopCapture() {
        ...
        // Release partial wakelock and wifi lock
        if (partialWakeLock.isHeld) {
            partialWakeLock.release()
            Log.d(logTag, "Partial wakelock released")
        }
        if (wifiLock.isHeld) {
            wifiLock.release()
            Log.d(logTag, "WiFi lock released")
        }
    }
    ...
    private fun initNotification() {
        ...
        notificationChannel = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channelId = "WanlianService"
            val channelName = "万联远程服务"
            val channel = NotificationChannel(
                channelId,
                channelName, NotificationManager.IMPORTANCE_HIGH
            ).apply {
                description = "万联远程服务通道"
            }
