...
class _RemotePageState extends State<RemotePage> with WidgetsBindingObserver {
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      trySyncClipboard();
      // When returning from background, request a video refresh to avoid stale frame
      if (gFFI.ffiModel.pi.isSet.isTrue) {
        sessionRefreshVideo(sessionId, gFFI.ffiModel.pi);
      }
    } else if (state == AppLifecycleState.paused ||
        state == AppLifecycleState.inactive) {
      // Keep wakelock active so that the connection stays alive in background
      // This prevents Android Doze mode from killing the network connection
      // which would trigger the 30-second timeout in io_loop.rs
      debugPrint("RemotePage: app going to background, keeping connection alive");
    }
  }
