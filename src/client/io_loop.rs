...
impl<T: InvokeUiSession> Remote<T> {
                    ...
                    tokio::select! {
                        ...
                        _ = self.timer.tick() => {
                            if last_recv_time.elapsed() >= SEC30 {
                                log::error!("Connection timeout! No data received for {:?}, breaking connection.", last_recv_time.elapsed());
    ...
    fn fps_control(&mut self, direct: bool, real_fps_map: HashMap<usize, i32>) {
        ...
        let mut fps_trending = |display: usize| {
            ...
            if len > 3 && last_auto_fps > limited_fps || len > std::cmp::max(3, decode_fps / 2) {
                ctl.idle_counter = 0;
                return Some(false);
            }
            if len <= 3 {
